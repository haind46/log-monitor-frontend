...
export default function UserHeading() {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess(data : any) {
      if (data?.message === "6002") {
        toast({
          variant: "destructive",
          title: t("system.form.errorTitle"),
        });
        return;
      }

      toast({     
        title: t("system.form.success"),
      });
  ...
  function deleteUsers() {
    confirm({
      title:  t("system.form.mutipleDelete"),
      description: `(${selectedUserIds.length}) `+ t("system.form.mutipleDeleteTitle"),
      async onOk() {
  ...
  return (
    <div className="w-full flex justify-between mb-4">
      <div className="font-bold text-xl">{t("main-nav.manageSystem")}</div>
